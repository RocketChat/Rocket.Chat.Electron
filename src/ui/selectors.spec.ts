import { selectGlobalBadge, selectGlobalBadgeCount, selectGlobalBadgeText } from './selectors';
...
describe('ui/selectors', () => {
  it('computes a numeric global badge from server badge counts', () => {
    ...
    expect(selectGlobalBadgeCount(state([{ badge: 2 }, { badge: '•' }]))).toBe(2);
  });
  ...
  it('returns undefined for no matching badges and empty badge text when blank', () => {
    expect(selectGlobalBadge(state([{ badge: null }, { badge: undefined }]))).toBeUndefined();
  ...
  it('renders badge text for number badge and count selector', () => {
    ...
    expect(selectGlobalBadgeCount(state([{ badge: '•' }, { badge: 7 }]))).toBe(7);
  });
});
