...
- CI (`.github/workflows/validate-pr.yml`) runs `yarn test:coverage` on PRs to `dev`, `master`, or a `release/*` line (Markdown-only PRs are skipped via `paths-ignore`) and uploads to **Codecov** (`unit` flag). The suite is split into two `--shard` jobs per OS; only the `ubuntu-24.04-arm` shards collect coverage (Windows/macOS run plain `yarn test`), and Codecov merges the two shard uploads for the same commit server-side. The per-shard `coverage-report-shard-N` artifacts are therefore partial — read the merged number from Codecov or from a local run, never from a single artifact. Coverage is **informational** — there is intentionally no hard `coverageThreshold` gate, matching the main Rocket.Chat monorepo's house style (`codecov.yml`: `patch: off`, project `target: auto / threshold: 1%`).
