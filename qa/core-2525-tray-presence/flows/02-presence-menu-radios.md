...
| 4    | Click a different radio item than the currently selected one, for example click `Busy` if `Online` was selected.                                                                                                                                                    | Target presence `busy` | The menu closes; re-opening the tray context menu (right-click again; on macOS: click the menu bar icon again) shows `Busy` now carrying the selected radio marker instead of the previous item.                    | Trigger the `click` handler for the target `PRESENCE_OPTIONS` entry and re-read the menu template.       |
