...
export class MemoryLeakDetector extends MemoryFeature {
  ...
  private analyzePatterns(): void {
    ...
    for (const [wcId, samples] of this.memorySamples.entries()) {
      ...
      if (pattern && pattern.confidence >= this.CONFIDENCE_THRESHOLD) {
        ...
        console.log(
          `[MemoryLeakDetector] Potential ${leak.type} leak detected in ${leak.url}`
        );
        console.log(`  Confidence: ${(leak.confidence * 100).toFixed(1)}%`);
        console.log(
          `  Growth rate: ${(leak.growthRate / 1024 / 1024).toFixed(2)} MB/min`
        );
      }
    }
  ...
  private detectLeakPattern(samples: MemorySample[]): {
  ...
  private detectSteadyGrowth(
    samples: MemorySample[]
  ): { rSquared: number; growthRate: number } | null {
  ...
  private detectRapidGrowth(
    samples: MemorySample[]
  ): { confidence: number; growthRate: number } | null {
    if (samples.length < 5) return null;

    const recentSamples = samples.slice(-5);
    const avgDelta =
      recentSamples.reduce((sum, s) => sum + Math.max(0, s.delta), 0) /
      recentSamples.length;

    const overallAvgDelta =
      samples.reduce((sum, s) => sum + Math.max(0, s.delta), 0) /
      samples.length;
    ...
    if (avgDelta > overallAvgDelta * this.RAPID_GROWTH_MULTIPLIER) {
      ...
      return {
        confidence: Math.min(
          1,
          avgDelta / (overallAvgDelta * this.RAPID_GROWTH_MULTIPLIER)
        ),
  ...
  private detectPlateauPattern(
    ...
    const postSpikeAvg =
      postSpikeSamples.reduce((sum, s) => sum + s.memory, 0) /
      postSpikeSamples.length;

    if (postSpikeAvg > avgMemory * 1.5) {
      // Check variation after spike (should be low for plateau)
      const postSpikeStdDev = Math.sqrt(
        postSpikeSamples.reduce(
          (sum, s) => sum + Math.pow(s.memory - postSpikeAvg, 2),
          0
        ) / postSpikeSamples.length
      );
      ...
      switch (leak.type) {
        case 'steady_growth':
          recommendations.push(
            `Reload tab "${leak.url}" - steady memory growth detected`
          );
        ...
        case 'rapid_growth':
          recommendations.push(
            `URGENT: Reload tab "${leak.url}" immediately - rapid memory growth`
          );
          recommendations.push(
            'Check for infinite loops or recursive allocations'
          );
          break;

        case 'sawtooth':
          recommendations.push(
            `Monitor tab "${leak.url}" - irregular memory pattern`
          );
  ...
  public async detectLeaks(): Promise<void> {
  ...
  public async analyzeWebContents(
    webContentsId: number
  ): Promise<LeakPattern | null> {
