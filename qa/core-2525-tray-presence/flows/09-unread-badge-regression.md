...
test_links: []
expected_result: The tray icon title/tooltip/badge overlay for unread mentions and messages behaves exactly as before this branch, regardless of the current presence value. On Windows and macOS, the tray icon itself never bakes in the unread numeral — the Windows taskbar overlay and the macOS menu-bar title already show the count. Linux keeps its numbered tray icons.
---
...
- On Windows/macOS, the resolved icon path varies with the unread count (e.g. contains a numeral or does not match the `presence-<presence>-notification` pattern) — the count must only ever appear via the title/taskbar overlay, never baked into the icon file. On Linux, the resolved icon path does not match the `presence-<presence>-notification-<n>` pattern at any step.
