...
function checkAvailableTools() {
  ...
  if (process.platform === 'win32') {
    // Use known chocolatey paths directly
...
/**
 * Sign Windows executables using jsign with Google Cloud KMS
 */
module.exports = async function signWithGoogleKms(config) {
  console.log('[winSignKms] Signing with jsign + Google Cloud KMS');
  ...
  let gcloudArgs = ['auth', 'print-access-token'];
  let gcloudExe = gcloudCmd;

  // On Windows, run through cmd.exe
  if (process.platform === 'win32') {
    gcloudArgs = ['/c', gcloudCmd, 'auth', 'print-access-token'];
    gcloudExe = 'cmd';
  }

  const gcloudResult = spawnSync(gcloudExe, gcloudArgs, {
    stdio: 'pipe',
    timeout: 30000,
    env: {
      ...process.env,
