...
import { Spellchecker, getAvailableDictionaries } from '@felixrieseberg/spellchecker';
...
const setupFallbackDictionaries = async () => {
	...
	if (userLanguage && await enableSpellCheckingDictionaries(userLanguage)) {
		return;
	}

	const navigatorLanguage = navigator.language;
	if (navigatorLanguage && await enableSpellCheckingDictionaries(navigatorLanguage)) {
		return;
	}

	await enableSpellCheckingDictionaries('en_US');
};
...
export default async () => {
	...
	webFrame.setSpellCheckProvider('', {
		spellCheck: (words, callback) => {
			setTimeout(() => {
				const misspelledWords = words.filter(isMisspelled);
