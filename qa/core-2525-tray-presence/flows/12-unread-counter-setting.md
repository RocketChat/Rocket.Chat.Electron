...
| 7 | From another account, send this user one @-mention so the tray shows the unread badge again. Then right-click the tray icon (Windows notification area / Linux system tray; on macOS left-click the app icon in the menu bar at the top-right) and click the last menu item, "Quit", to exit fully. Start the app again, then press `Ctrl+,` / `Cmd+,` to reopen the settings window on its "General" section. | 1 unread @-mention, restart | "Unread counter in tray icon" is still on, and the tray icon shows the unread badge for 1 unread (red circle with `1` on Windows/Linux, the dot glyph on macOS), not the presence bullet. | Read the persisted value from the store after restart (`true`) and assert the resolved path is `notification-1.*` / `notificationTemplate.png`. |
