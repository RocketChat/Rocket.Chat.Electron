...
export const clearStaleAssets = async (
  releaseId: number,
...
export const forceCleanOldAssets = async (
  releaseId: number,
  ...
  core.info(
    `Force cleaning ${assetsToDelete.length} old assets, keeping latest ${keepLatest} assets`
  );

  for (const asset of assetsToDelete) {
    core.info(
      `deleting old asset ${asset.name} (created: ${asset.created_at})`
    );
    await octokit.request(
      'DELETE /repos/{owner}/{repo}/releases/assets/{asset_id}',
      {
