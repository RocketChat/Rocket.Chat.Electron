...
export const writeCertFromSecret = async (
  secretValue: string,
  fileName: string
): Promise<string> => {
...
export const verifyCertificateInStore = async (
  certSha1: string
): Promise<void> => {
  ...
  if (!certCheck || certCheck.trim() === '') {
    throw new Error(
      `Certificate with thumbprint ${certSha1} not found in CurrentUser\\My store!`
    );
  }
...
export const setupCertificates = async (): Promise<string> => {
  ...
  const userCertPath = await writeCertFromSecret(
    core.getInput('win_user_crt'),
    'user.crt'
  );
  const intermediateCertPath = await writeCertFromSecret(
    core.getInput('win_intermediate_crt'),
    'intermediate.crt'
  );
  const rootCertPath = await writeCertFromSecret(
    core.getInput('win_root_crt'),
    'root.crt'
  );
  ...
  if (!certSha1) {
    throw new Error(
      'Could not determine certificate thumbprint. Provide win_kms_cert_sha1 or win_user_crt input.'
    );
  }
