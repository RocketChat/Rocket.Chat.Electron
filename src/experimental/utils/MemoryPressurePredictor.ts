...
export class MemoryPressurePredictor {
  ...
  private calculateGrowthRate(): number {
    ...
    const recentPoints = this.dataPoints.slice(
      -Math.min(20, this.dataPoints.length)
    );
  ...
  private calculateVolatility(): number {
    ...
    const variance =
      memories.reduce((sum, m) => sum + Math.pow(m - mean, 2), 0) /
      memories.length;
  ...
  private assessRisk(
    ...
    if (
      currentPercent >= this.config.criticalThreshold ||
    ...
    if (
      currentPercent >= this.config.warningThreshold ||
    ...
    if (
      predictedPercent >= this.config.warningThreshold ||
...
class TrendModel {
  ...
  update(point: MemoryDataPoint): void {
    if (this.smoothedValue === 0) {
      ...
      this.smoothedValue =
        this.alpha * point.memory + (1 - this.alpha) * this.smoothedValue;
...
class SeasonalModel {
  ...
  update(point: MemoryDataPoint): void {
    ...
    this.weeklyPattern[dayOfWeek] =
      (this.weeklyPattern[dayOfWeek] * this.weeklyCount[dayOfWeek] +
        point.memory) /
...
class AnomalyDetector {
  ...
  update(point: MemoryDataPoint): void {
    ...
    if (this.count > 1) {
      ...
      this.variance =
        ((this.count - 2) * this.variance + delta * delta2) / (this.count - 1);
