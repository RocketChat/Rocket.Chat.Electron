...
| 2 | Minimize or hide the main window, then right-click the tray icon (Windows notification area / Linux system tray) to open the context menu. | none | The menu's first item reads `Sign in to <workspace name or URL>` (matching the workspace's configured title, or its URL if no title is set), rendered as a normal enabled item with no radio marker. No `Online`/`Away`/`Busy`/`Offline` items and no custom-status line appear above or below it. | Read the tray menu template and assert the presence-section items array has exactly one entry with that label pattern and `enabled: true`, and no `type: 'radio'` items. |
