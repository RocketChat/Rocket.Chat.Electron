...
export class PerformanceCollector extends MemoryFeature {
  ...
  protected async onEnable(): Promise<void> {
    ...
    console.log('[PerformanceCollector] Started collecting performance metrics');
  }
  ...
  private async buildSnapshot(): Promise<PerformanceSnapshot> {
    ...
    const cpuPercent = ((cpuUsage.user + cpuUsage.system) / 1000000) * 100 / (this.COLLECTION_INTERVAL / 1000);
  ...
  private async collectRenderMetrics(windows: BrowserWindow[]): Promise<PerformanceSnapshot['render']> {
  ...
  private async collectWebContentsMetrics(): Promise<PerformanceSnapshot['webContents']> {
    const allWebContents = webContents.getAllWebContents();
    ...
    for (const wc of allWebContents) {
      ...
      try {
        ...
        const domMetrics = await wc.executeJavaScript(`
          ({
  ...
  private detectAnomalies(snapshot: PerformanceSnapshot): void {
    ...
    // High memory usage
    const memoryPercent = ((snapshot.memory.system.total - snapshot.memory.system.free) / snapshot.memory.system.total) * 100;
  ...
  private addAnomaly(anomaly: PerformanceReport['anomalies'][0]): void {
    ...
    console.log(`[PerformanceCollector] Anomaly detected: ${anomaly.description}`);
  ...
  public generateReport(): PerformanceReport {
    ...
    const networkBytes = this.snapshots.reduce((sum, s) => 
      sum + s.network.bytesReceived + s.network.bytesSent, 0
    );
  ...
  private generateRecommendations(params: any): string[] {
    ...
    if (params.peakCPU > 95) {
      recommendations.push('CPU spikes detected. Check for blocking operations or infinite loops.');
    }
    
    if (params.avgMemory > 1000 * 1024 * 1024) {
      recommendations.push('High memory usage. Consider implementing memory cleanup strategies.');
    }
    
    if (params.memoryLeakSuspected) {
      recommendations.push('Potential memory leak detected. Review object lifecycle and event listeners.');
    }
    
    if (params.avgFPS < 45) {
      recommendations.push('Low frame rate detected. Optimize rendering and reduce DOM manipulations.');
    }
    
    if (params.minFPS < 20) {
      recommendations.push('Severe frame drops detected. Profile rendering performance.');
    }
  ...
  private async saveReport(): Promise<void> {
    try {
      ...
      const reportPath = path.join(app.getPath('userData'), 'performance-reports');
  ...
  public exportCSV(): string {
    const headers = ['timestamp', 'cpu_usage', 'memory_heap', 'memory_rss', 'fps', 'event_loop_lag'];
