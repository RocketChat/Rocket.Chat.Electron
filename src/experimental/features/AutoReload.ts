...
export class AutoReload extends MemoryFeature {
  ...
  private async checkAllWebContents(): Promise<void> {
    ...
    console.log(
      `[AutoReload] 🔍 Checking ${this.webContentsStates.size} WebContents for memory issues`
    );
    ...
    for (const [url, state] of this.webContentsStates) {
      ...
      const metric = appMetrics.find(
        (m) => (m as any).webContents?.id === state.webContents.id
      );
      if (!metric) continue;
      ...
      if (currentMemoryGB > 2.5) {
        console.log(
          `[AutoReload] 📈 ${url}: ${currentMemoryGB.toFixed(2)}GB (approaching limit)`
        );
      }
      ...
      if (state.memoryHistory.length >= 3) {
        ...
        if (state.growthRate > 0) {
          const remainingMemory =
            this.MEMORY_CRITICAL_THRESHOLD - currentMemory;
          state.predictedCrashTime =
            Date.now() + (remainingMemory / state.growthRate) * 60000;
        } else {
      ...
      const shouldReload = await this.shouldReloadWebContents(
        state,
        currentMemory
      );
  ...
  private async shouldReloadWebContents(
    ...
    if (currentMemory >= this.MEMORY_CRITICAL_THRESHOLD) {
      console.warn(
        `[AutoReload] 🚨 CRITICAL: ${state.url} at ${(currentMemory / 1024 / 1024 / 1024).toFixed(2)}GB - MUST RELOAD!`
      );
      return true;
    }
    ...
    if (
      currentMemory >= this.MEMORY_WARNING_THRESHOLD &&
      state.growthRate > this.MEMORY_GROWTH_RATE_THRESHOLD
    ) {
      const growthMBPerMin = state.growthRate / 1024 / 1024;
      console.warn(
        `[AutoReload] ⚠️ WARNING: ${state.url} at ${(currentMemory / 1024 / 1024 / 1024).toFixed(2)}GB, growing ${growthMBPerMin.toFixed(1)}MB/min`
      );
      return true;
    }
    ...
    if (
      state.predictedCrashTime &&
      state.predictedCrashTime - now < 5 * 60000
    ) {
      const minutesUntilCrash = (state.predictedCrashTime - now) / 60000;
      console.warn(
        `[AutoReload] 🔮 PREDICTION: ${state.url} will crash in ${minutesUntilCrash.toFixed(1)} minutes!`
      );
      return true;
    }
    ...
    if (systemPressure > 0.9 && currentMemory > this.MEMORY_WARNING_THRESHOLD) {
      console.warn(
        `[AutoReload] 💥 SYSTEM PRESSURE: ${(systemPressure * 100).toFixed(1)}% memory used, reloading ${state.url}`
      );
      return true;
    }
  ...
  private async reloadWebContents(
    ...
    console.log(
      `[AutoReload] 🔄 RELOADING ${state.url} - Memory: ${(memoryBefore / 1024 / 1024 / 1024).toFixed(2)}GB`
    );
  ...
  private showReloadNotification(url: string): void {
    ...
    dialog.showMessageBox({
      ...
      detail:
        'The experimental auto-reload protection feature detected high memory usage and reloaded the tab to maintain stability.',
      buttons: ['OK'],
      noLink: true,
    });
  }
